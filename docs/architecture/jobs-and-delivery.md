...
how execution and delivery are tracked separately, what a restart reconciles, and what a
graceful stop does and does not do to a live row. It does not cover what a finished
delegation still owes its creator — the announcement, the answer retained for it, and how
long the row is kept ([`architecture/delegation-announcements.md`](delegation-announcements.md)) —
how a job's turn runs (the turn loop), the leased
...
its entry into recovery; whether anyone is ever told is INV-JOB-010's subject
([`delegation-announcements.md`](delegation-announcements.md)), and the two
