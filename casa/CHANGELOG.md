...
- Deleting a webhook trigger now retires the secret Casa generated for it, on
  the reload or restart that removes the trigger's route. Until now that
  secret stayed behind through every way a trigger could be removed, and a
  trigger later created under the same name quietly took it over, so a caller
  still holding the old value kept authenticating against the new trigger. A
  trigger recreated under a name that was used before is now given a fresh
  secret; hand it to the caller again.
- A Casa-owned trigger's route now accepts only a secret Casa can prove it
  generated for that trigger. A value Casa cannot prove — a file placed by
  hand in a slot Casa owns, for example — is refused (`401`) and is never
  deleted.

### Changed

- Upgrade note: a secret generated by a version of Casa that kept no proof of
  generating it is refused (`401`) and reported as `unproven_blocked` in the
  reload report, but it is never deleted. The remedy is to delete
  `/data/webhook_secrets/<trigger-name>` on the host, reload, and point the
  caller at the new secret. Every Casa-owned per-trigger secret that exists
  before this update needs that step once.

### Documentation

- The configuration document now says what a `scope=agent` reload of a
  disabled specialist does: it tears that specialist down — its consumer, its
  in-flight work, its grants, its scheduled asks, its triggers and its webhook
  routes — and constructs nothing; its webhook secrets stay on disk.
